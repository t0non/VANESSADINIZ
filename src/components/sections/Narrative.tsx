    ...
    <section className="relative w-full overflow-hidden bg-[#FBFBF9] pt-0 pb-24 lg:py-0">
      <div className="grid grid-cols-1 lg:grid-cols-2 items-center">
        ...
        <div className="hidden lg:flex relative h-screen items-end justify-center overflow-visible">
          <div className="relative w-full h-full flex items-end -translate-y-40 [mask-image:linear-gradient(to_bottom,black_10%,black_40%,transparent_90%)]">
            <Image
          ...
          <FadeIn>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-16">
              {specialties.map((item, index) => (
                <div 
                  key={index} 
                  className="flex items-center gap-4 bg-white px-8 h-20 rounded-full shadow-sm border border-gray-100 transition duration-300 hover:shadow-md hover:border-[#C1A68D]/20 group"
                >
                  <div className="flex-shrink-0 text-[#2A3F32] group-hover:text-[#C1A68D] transition-colors duration-300">
                    {item.icon}
                  </div>
                  <span className="font-body text-[13px] md:text-base text-[#2A3F32] font-medium leading-tight">
                    {item.text}
                  </span>
                </div>
              ))}
